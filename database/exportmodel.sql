...
CREATE TABLE IF NOT EXISTS `u892058990_AOMJK`.`audit06` (
  ...
  `idrol` INT NOT NULL,
  PRIMARY KEY (`idprof`),
  INDEX `fk_profile_rol1_idx` (`idrol` ASC) ,
  CONSTRAINT `fk_profile_rol1`
    FOREIGN KEY (`idrol`)
    REFERENCES `u892058990_AOMJK`.`audit05` (`idrol`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `u892058990_AOMJK`.`users` (
  ...
  INDEX `fk_users_profile1_idx` (`idprof` ASC) ,
  INDEX `fk_users_persons1_idx` (`idpers` ASC) ,
  CONSTRAINT `fk_users_profile1`
    FOREIGN KEY (`idprof`)
    REFERENCES `u892058990_AOMJK`.`audit06` (`idprof`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `u892058990_AOMJK`.`audit02` (
  `idperm` INT NOT NULL,
  `action` ENUM('C', 'R', 'U', 'D') NOT NULL,
  `idmod` INT NOT NULL,
  `idrol` INT NOT NULL,
  PRIMARY KEY (`idperm`),
  INDEX `fk_permissions_modules1_idx` (`idmod` ASC) ,
  INDEX `fk_permissions_rol1_idx` (`idrol` ASC) ,
  CONSTRAINT `fk_permissions_modules1`
    FOREIGN KEY (`idmod`)
    REFERENCES `u892058990_AOMJK`.`audit03` (`idmod`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_permissions_rol1`
    FOREIGN KEY (`idrol`)
    REFERENCES `u892058990_AOMJK`.`audit05` (`idrol`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `u892058990_AOMJK`.`audit04` (
  `idsubmod` INT NOT NULL,
  `detsubmod` VARCHAR(45) NOT NULL,
  `idmod` INT NOT NULL,
  PRIMARY KEY (`idsubmod`),
  INDEX `fk_submodules_modules1_idx` (`idmod` ASC) ,
  CONSTRAINT `fk_submodules_modules1`
    FOREIGN KEY (`idmod`)
    REFERENCES `u892058990_AOMJK`.`audit03` (`idmod`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `u892058990_AOMJK`.`audit11` (
  `iddirecxfil` INT NOT NULL,
  `iddirectors` INT NOT NULL,
  `idfilms` INT NOT NULL,
  PRIMARY KEY (`iddirecxfil`),
  INDEX `iddirectors_idx` (`iddirectors` ASC) ,
  INDEX `idfilms_idx` (`idfilms` ASC) IN,
  CONSTRAINT `iddirectors2`
